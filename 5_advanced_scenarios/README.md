...
* [Understand how to onboard non-Kubernetes workloads to the mesh](#understand-how-to-onboard-non-kubernetes-workloads-to-the-mesh)
* [Troubleshoot configuration issues](#troubleshoot-configuration-issues)
...
### Ex:1 Onboard non-Kubernetes workloads to the mesh

Requirements:
* Inject a Virtual Machine to be part of the mesh.
  * Network connectivity from the Virtual Machine to the Istio east west gateway.
* Check that the Virual Machine can reach any injected service in the mesh.
...
<details>
  ...
  # Create a dedicated East West Gateway
  $ cat <<EOF > istio-east-west-gateway.yaml
...
EOF
  ...
  $ istioctl install -f istio-east-west-gateway.yaml

  # You should have already a candidate Virtual Machine created to onboard on the Istio Mesh
  ...
  # Expose IstioD services inside the cluster via the east-west gateway: Gateway & VirtualService
  $ cat <<EOF > expose-istiod.yaml
...
EOF
  ...
  # Before proceeding to generate the istio-token, as part of istioctl x workload entry, you should verify third party tokens are enabled in your cluster by following the steps describe here. 
  ...
  $ scp mesh.yaml cluster.env root-cert.pem istio-token hosts <Virtual-Machine-IP>:/tmp

  # Shell inside the Virual Machine instance to perform the following comands:
  $ ssh<Virtual-Machine-IP>

  # Install the root certificate at /etc/certs:
  $ sudo mkdir -p /etc/certs
  $ sudo cp -p /tmp/root-cert.pem /etc/certs/root-cert.pem

  # Install the token at /var/run/secrets/tokens:
  $ sudo  mkdir -p /var/run/secrets/tokens
  $ sudo cp -p /tmp/istio-token /var/run/secrets/tokens/istio-token

  # Install the package containing the Istio virtual machine integration runtime with the same version that the IstioD Control Plane on K8S:
  $ curl -LO https://storage.googleapis.com/istio-release/releases/1.19.0/deb/istio-sidecar.deb
  $ sudo dpkg -i istio-sidecar.deb

  # Install cluster.env within the directory /var/lib/istio/envoy/:
  $ sudo cp -p /tmp/cluster.env /var/lib/istio/envoy/cluster.env

  # Install the Mesh Config to /etc/istio/config/mesh:
  $ sudo cp -p /tmp/mesh.yaml /etc/istio/config/mesh

  # Add the istiod host to /etc/hosts:
  $ sudo sh -c 'cat $(eval echo ~$SUDO_USER)/hosts >> /etc/hosts'
  # If the hosts file is generated empty try:
  $ echo "<Istio east-west gateway IP> istiod.istio-system.svc" >> /etc/hosts
  ...
  # Check that the new Virual Machine is in SYNC with IstioD
  $ istioctl ps |grep vm-mysqldb
XXXXXXX.vm-mysqldb     Kubernetes     SYNCED     SYNCED     SYNCED     SYNCED       NOT SENT     istiod-695b888b6-8wkn5     1.19.0

# Deploy the httpbin service:
$ kubectl apply -n istio-system -f samples/httpbin/httpbin.yaml
